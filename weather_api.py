...
try:
    from weather_processor import discover_available_runs, simple_process, parse_timestamp_flexible
    ...
    try:
        ...
        for run in runs:
            formatted_runs.append({
                ...
                'variables': ['TOT_PREC', 'VMAX_10M']  # Both variables always available
            })
...
def start_processing():
    ...
    try:
        ...
        thread = threading.Thread(
            target=_process_runs,
            args=(job_id, selected_runs)
        )
        thread.daemon = True
        thread.start()
        
        logger.info(f"🚀 Started processing job {job_id} with {len(selected_runs)} runs")
...
def get_specific_run_data(run_id):
    ...
    try:
        ...
        # Strip first timestamp from all variables for (T0, T1] interval notation
        ...
        if 'TOT_PREC' in data.get('variables', {}):
            ...
            data['variables'] = add_derived_precipitation_variables(data['variables'], ensemble_data)
...
def add_derived_precipitation_variables(variables: Dict, ensembles: Dict = None) -> Dict:
    ...
    if ensembles and 'TOT_PREC' in ensembles:
        ...
        for ensemble in ensembles['TOT_PREC']:
            if 'accumulated_values' in ensemble and len(ensemble['accumulated_values']) > 0:
                # Strip first value to match (T0, T1] interval notation
                accum_vals = ensemble['accumulated_values'][1:] if len(ensemble['accumulated_values']) > 1 else ensemble['accumulated_values']
    ...
    for key, values in stats.items():
        if isinstance(values, list) and len(values) > 0:
            ...
            if remainder != 0:
                ...
                values_array = np.pad(values_array, (0, padding_size), mode='constant', constant_values=0)
            ...
            hourly_stats[key] = hourly_sums[:len(times) // steps_per_hour + (1 if len(times) % steps_per_hour else 0)].tolist()
...
def strip_first_timestamp(variables: Dict) -> Dict:
    """Strip first timestamp from all variables to use (T0, T1] interval notation using vectorized operations"""
    import numpy as np
    ...
    for var_name, var_data in variables.items():
        if not isinstance(var_data, dict):
            stripped_variables[var_name] = var_data
            continue
            
        # Copy variable data
        stripped_var_data = var_data.copy()
        
        # Strip first timestamp if present
        if 'times' in stripped_var_data and len(stripped_var_data['times']) > 1:
            stripped_var_data['times'] = stripped_var_data['times'][1:]
        
        # Strip first value from ensemble statistics using vectorized operations
        if 'ensemble_statistics' in stripped_var_data:
            stripped_stats = {}
            
            # Process all statistics together using vectorized operations where possible
            stats = stripped_var_data['ensemble_statistics']
            for stat_key, stat_values in stats.items():
                if isinstance(stat_values, list) and len(stat_values) > 1:
                    # Use numpy slicing for efficiency
                    stripped_stats[stat_key] = np.array(stat_values)[1:].tolist()
                else:
                    stripped_stats[stat_key] = stat_values
                    
            stripped_var_data['ensemble_statistics'] = stripped_stats
        
        stripped_variables[var_name] = stripped_var_data
    
    return stripped_variables
...
def _process_runs(job_id: str, selected_runs: List[str]):
    ...
    try:
        ...
        logger.info(f"🎉 Job {job_id} completed: {success_count}/{len(selected_runs)} runs successful")
...
def trigger_cleanup():
    ...
    try:
        if not CLEANUP_AVAILABLE:
            return jsonify({
                'success': False,
                'error': 'Cleanup module not available'
            }), 500
        
        # Get retention hours from request or use default
        data = request.get_json() or {}
        retention_hours = data.get('retention_hours', 12)
...
def startup_cleanup():
    ...
    try:
        ...
        if result.get('total_files_deleted', 0) > 0 or result.get('total_directories_deleted', 0) > 0:
            space_mb = result.get('total_space_freed_bytes', 0) / (1024**2)
            logger.info(f"✅ Startup cleanup completed: "
    ...
    while True:
        try:
            ...
            if result.get('total_files_deleted', 0) > 0:
                space_mb = result.get('total_space_freed_bytes', 0) / (1024**2)
                logger.info(f"✅ Background cleanup completed: "
...
if __name__ == "__main__":
    ...
    if not PROCESSOR_AVAILABLE:
        logger.error("❌ Weather processor not available. Please ensure weather_processor.py is working.")
        sys.exit(1)
